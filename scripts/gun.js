...
function shootBullet(position, velocity, grenade) {
  ...
  var smokeVelocity = Vec3.multiply(-1, velocity);
...
MyAvatar.attach(gunModel, "RightHand", {
  x: 0.04,
  y: 0.22,
  z: 0.02
}, Quat.fromPitchYawRollDegrees(-172, -85, 79), 0.40);
MyAvatar.attach(gunModel, "LeftHand", {
  x: -0.04,
  y: 0.22,
  z: 0.02
}, Quat.fromPitchYawRollDegrees(-172, 85, -79), 0.40);
  ...
  if (numberOfTriggers == 2 && controllersPerTrigger == 2) {
    for (var t = 0; t < 2; t++) {
      ...
      if (shootABullet) {
        ...
        var position = {
          x: fingerTipPosition.x + palmToFingerTipVector.x / 2,
...
function scriptEnding() {
  ...
  MyAvatar.detachOne(gunModel);
  clearPose();
  Entities.deleteEntity(smokeEffect);
}
